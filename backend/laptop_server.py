...
# Load YOLO models
model_obj = YOLO("../models/yolo26s.pt").to(device)    # Standard Objects
model_curr = YOLO("../models/yolo26_seg.pt").to(device) 
current_mode = "OBJECT"
...
async def process_llm(labels, mode):
    ...
    llm_busy = True
    print(f"\n[LLM Input]: Processing labels: {labels}")
        ...
        try:
            while radxa_connected_event.is_set():
                ...
                if labels and not llm_busy:
                    ...
                    print(f"[Detections]: {unique_labels}")
