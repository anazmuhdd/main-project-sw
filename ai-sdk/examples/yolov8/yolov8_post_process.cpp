...
int yolov8_post_process(float* data, int width, int height, det_res_t* res) {
    ...
    // Channels 0-3: bbox coords (cx, cy, w, h)
    // Channels 4-83: classification scores
    ...
    int det_num = std::min((int)detections.size(), (size_t)64);
    res->num = det_num;
    for (int i = 0; i < det_num; i++) {
        res->results[i].rel_box.x = detections[i].x1 / (float)width;
        res->results[i].rel_box.y = detections[i].y1 / (float)height;
        res->results[i].rel_box.w = (detections[i].x2 - detections[i].x1) / (float)width;
        res->results[i].rel_box.h = (detections[i].y2 - detections[i].y1) / (float)height;
