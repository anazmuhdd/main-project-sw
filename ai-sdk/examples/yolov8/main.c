...
// External pre-process function from yolov8_pre_process.cpp
...
int main(int argc, char** argv) {
    ...
    unsigned char* input_data = yolov8_pre_process(image_path, &input_size);
    if (input_data == NULL) {
    ...
    void* npu_output_ptr = vip_map_buffer(output_buffer);
    det_res_t results;
    memset(&results, 0, sizeof(det_res_t));
    
    // Call our NMS-based post-process
    yolov8_post_process((float*)npu_output_ptr, 640, 640, &results);
